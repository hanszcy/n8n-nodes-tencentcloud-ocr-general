...
import { TencentCloudOCRGeneral } from './TencentCloudOCRGeneral';

export class TencentCloudOCRGeneralNode implements INodeType {
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				ocr = new TencentCloudOCRGeneral(
				...
				switch (operation) {
					...
					case 'table':
						...
						break;
					case 'idCard':
						const cardSide = this.getNodeParameter('cardSide', i) as string;
